...
export const createRemisionSchema = z
  ...
  .refine(
    (data) => data.type !== 'priced' || data.items.every((i) => typeof i.unitPrice === 'number'),
    { message: 'Cada item debe tener unitPrice cuando type es "priced"', path: ['items'] }
  );
...
export const updateRemisionSchema = z.object({
  ...
  ivaPercentage: z.number().min(0).max(100).optional(),
  notes: z.string().trim().max(500).optional(),
  driverId: z.string().regex(/^[a-fA-F0-9]{24}$/).optional(),
  clientId: z.string().regex(/^[a-fA-F0-9]{24}$/).optional(),
});
