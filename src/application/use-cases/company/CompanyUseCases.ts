...
export class CompanyUseCases {
  ...
  async create(dto: CreateCompanyDto, ownerId: string): Promise<Company> {
    ...
    return this.companyRepo.create({ ...dto, ownerId, logoUrl: dto.logoUrl ?? null });
  }

  async getById(id: string, requesterId: string, role: 'admin' | 'user'): Promise<Company> {
  ...
  private assertOwnership(company: Company, requesterId: string, role: 'admin' | 'user') {
