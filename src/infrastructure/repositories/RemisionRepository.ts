...
import { RemisionModel, RemisionDocument } from '../database/models/Remision.model.js';
...
export class RemisionRepository implements IRemisionRepository {
  async create(data: Omit<Remision, 'id' | 'createdAt' | 'updatedAt'>): Promise<Remision> {
  ...
  async update(id: string, data: Partial<Remision>): Promise<Remision | null> {
    const doc = await RemisionModel.findByIdAndUpdate(id, data, { new: true, runValidators: true });
    return doc ? toDomain(doc) : null;
  }
  ...
  async getNextConsecutive(companyId: string): Promise<number> {
    const last = await RemisionModel.findOne({ companyId }).sort({ consecutive: -1 }).select('consecutive');
