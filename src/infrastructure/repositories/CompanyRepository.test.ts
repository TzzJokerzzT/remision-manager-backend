// Co-located unit tests for CompanyRepository.listByOwner pagination.
...
function expectedDomain(id: string): Company {
	return {
		id,
		name: "Acme",
		nit: "900123456",
...
describe("CompanyRepository.listByOwner", () => {
	test("applies skip and limit derived from page and returns { items, total }", async () => {
		...
		expect(findMock).toHaveBeenCalledWith({ ownerId: "owner1" });
		expect(query.skip).toHaveBeenCalledWith(20);
		expect(query.limit).toHaveBeenCalledWith(20);
		expect(countDocumentsMock).toHaveBeenCalledWith({ ownerId: "owner1" });
	...
	test("countDocuments receives the same filter including $text when search present", async () => {
		...
		const repo = new CompanyRepository();
		await repo.listByOwner("owner1", "foo", { limit: 20, page: 1 });
